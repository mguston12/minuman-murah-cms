  ...
  <div>
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
      <div>
        ...
        <p class="text-muted mb-0">Update your voucher details</p>
      </div>
      ...
      <div class="card-body">
        ...
        <form @submit.prevent="handleSubmit">
          <div class="row g-3">
            ...
            <div class="col-md-6">
              <label class="form-label fw-medium"
                >Is Publish <span class="text-danger">*</span></label
              >
...
import type { VoucherUpdatePayload, VoucherCategory } from "~/types/voucher";
...
const categories = ref<VoucherCategory[]>([]);
const selectedCategories = ref<VoucherCategory[]>([]);

const form = reactive<VoucherUpdatePayload>({
  name: "",
  code: "",
  description: null,
  status: "ACTIVE",
...
const discountStep = computed(() => {
  return form.discount_type === "PERCENTAGE" ? 1 : 0.01;
});
...
const formatDateTimeLocal = (value: string | null) => {
  ...
  const dt = DateTime.fromISO(value, { zone: "Asia/Jakarta" });
...
const loadVoucher = async () => {
  ...
  const { data, error } = await getAdminVoucher(id);

  if (error) {
    message.value = error.message || "Failed to load voucher";
  ...
  if (voucher.categories && Array.isArray(voucher.categories)) {
    selectedCategories.value = voucher.categories;
    form.fk_category_id = voucher.categories.map((c: any) => c.id);
  }

  form.name = voucher.name;
  form.code = voucher.code;
  form.description = voucher.description;
  form.status = voucher.status ?? "ACTIVE";
  form.limit_user = voucher.limit_user;
  form.start_date =
    voucher.start_date && formatDateTimeLocal(voucher.start_date);
  form.end_date = voucher.end_date && formatDateTimeLocal(voucher.end_date);
  form.min_purchase = voucher.min_purchase ?? 0;
  form.is_published = voucher.is_published ?? true;
  form.discount_type = voucher.discount_type ?? "FIXED";
  if (form.discount_type === "PERCENTAGE") {
    form.discount_value = Math.round(Number(voucher.discount_value));
  } else {
    form.discount_value = Number(voucher.discount_value ?? 0);
  }
...
const handleSubmit = async () => {
  ...
  const id = Number(route.params.id);
  const { data, error } = await updateVoucher(id, form);

  if (form.discount_type === "PERCENTAGE") {
    form.discount_value = Math.round(form.discount_value);
  }

  if (error) {
    message.value = error.message || "Failed to update voucher";
    errors.value = error.errors || {};
    toast.error(message.value);
  } else {
    message.value = "Voucher updated successfully!";
