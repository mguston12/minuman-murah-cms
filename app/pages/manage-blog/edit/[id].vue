                  ...
                  <tr v-for="cat in categories" :key="cat.id">
                    <td>
                      ...
                      <template v-else>
                        ...
                        <small v-if="cat.description" class="text-muted">{{
                          cat.description
                        }}</small>
                      </template>
                    </td>
...
const loadBlog = async () => {
  ...
  if (data.data?.blogs) {
    blog =
      data.data.blogs.find((b: Blog) => b.id.toString() === blogId.value) ||
      null;
  }
  ...
  if (target) {
    ...
    if (parent) {
      ...
      placeholder.innerHTML =
        '<i class="bi bi-image text-muted" style="font-size: 2rem;"></i><p class="text-muted mb-0 mt-2 small">Image not available</p>';
